...
public class WebPage {
    ...
    public void createHTMLTree() {
        ...
        for (int i = 0; i<HTMLAL.size(); i++) {
            if (HTMLAL.get(i).equals("<")) {
                ...
                if (isHTMLElement(element.toString())) {
    ...
    public ArrayList<String> findTags() {
        ...
        StringBuilder elementTag = new StringBuilder();
        for (int i=0; i<HTMLAL.size(); i++) {
            if (HTMLAL.get(i).equals("<")) {
                elementTag.append("<");
                while (!HTMLAL.get(i).equals(">")) {
                    elementTag.append(HTMLAL.get(++i));
                }
                tagArrayList.add(elementTag.toString());
                elementTag.setLength(0);
            }
        }
        return tagArrayList;
    }

    private boolean isHTMLElement(String tag) {
        String[] notCrumbElements = {"!", "/", "input", "link"};
        boolean temp = true;
        for (String element : notCrumbElements) {
            if (tag.startsWith(element, 1)) {
                temp = false;
                break;
            }
        }
